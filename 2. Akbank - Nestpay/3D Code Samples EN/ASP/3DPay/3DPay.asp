...
<!-- #include file = "hex_sha1_js.asp" -->
    ...
    Dim clientId,amount,oid,okUrl,failUrl,rndval,instalment,transactionType,storekey,hashstr,hash1,hash,storetype, currencyval, lang
    
    clientId = "400269820"  		'Merchant Id defined by bank to user
    amount = "9.95"         		'Transaction amount
    oid = ""                		'Order Id. Must be unique. If left blank, system will generate a unique one.
    okUrl = "http://localhost:53024/3DPayResult.asp"    			'URL which client be redirected if authentication is successful
    failUrl = "http://localhost:53024/3DPayResult.asp"  			'URL which client be redirected if authentication is not successful
    rndval = now()						'A random number, such as date/time
    currencyval = "949"                'Currency code, 949 for TL, ISO_4217 standard
    lang = ""                         'Language parameter, "tr" for Turkish (default), "en" for English
    
    instalment = ""					'Instalment count, if there's no instalment should left blank
    transactionType = "Auth"		'transaction type			
    
    storekey = "400DX9820"				'Store key value, defined by bank.
    storetype = "3d_pay" 				'3D authentication model
    
    hashstr = clientId & oid & amount & okUrl & failUrl & transactionType & instalment & rndval & storekey
